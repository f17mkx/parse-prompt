...
- **`lib/routing.py`** — `route_text()` per item + `route_prompt()` for the UserPromptSubmit hook. Scores capabilities against a SQLite "routing brain" that adjusts keyword weights based on what gets invoked.
